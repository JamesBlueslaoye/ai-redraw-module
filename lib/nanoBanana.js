...
async function redrawViaNanobananaAi({
  ...
  try {
    ...
    throwApiError(`Nano Banana 网络请求失败: ${detail}`, 502);
  }
  const data = await res.json().catch(() => ({}));
...
async function redrawViaVolcengine({ imageBase64, mimeType, prompt, negativePrompt, size, resolution, sourceWidth, sourceHeight }) {
  ...
  try {
    ...
    const detail = error instanceof Error ? error.message : String(error);
    throwApiError(`火山方舟网络请求失败: ${detail}`, 502);
  }
...
async function redrawViaGoogle({ imageBase64, mimeType, prompt, negativePrompt }) {
  ...
  try {
    ...
    const detail = error instanceof Error ? error.message : String(error);
    throwApiError(`Google Gemini 网络请求失败: ${detail}`, 502);
  }
...
async function redrawViaProxy({ imageBase64, mimeType, prompt, negativePrompt }) {
  ...
  try {
    ...
    const detail = error instanceof Error ? error.message : String(error);
    throwApiError(`代理线路网络请求失败: ${detail}`, 502);
  }
...
async function fetchImageFromUrl(imageUrl) {
  ...
  const mimeType = resolveImageMimeType(buffer, headerMime);
  if (!mimeType) {
    const preview = buffer.toString('utf8', 0, 120).replace(/\s+/g, ' ').trim();
    throwApiError(`下载到的内容不是图片：${preview || '无可识别内容'}`, 502);
  }
  return {
...
function resolveImageMimeType(buffer, headerMime) {
  ...
  if (headerMime) {
    if (headerMime === 'application/octet-stream') return null;
    if (headerMime.startsWith('text/') || headerMime.includes('json') || headerMime.includes('html')) return null;
  }
