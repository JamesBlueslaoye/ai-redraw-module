  ...
  for (const target of uploadTargets) {
    for (let attempt = 1; attempt <= target.attempts; attempt += 1) {
      try {
        ...
        errors.push(`${target.name}#${attempt}:${res.status}:${text.slice(0, 80) || '无响应'}`);
      } catch (err) {
        errors.push(`${target.name}#${attempt}:ERR:${err.message || '请求失败'}`);
      }
    }
  }
...
async function waitNanobananaTask(apiKey, taskId, { timeoutMs = 300000, intervalMs = 3000 } = {}) {
  ...
  while (Date.now() - started < timeoutMs) {
    const res = await fetch(`${base}/record-info?taskId=${encodeURIComponent(taskId)}`, {
